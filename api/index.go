...
func handleAttendance(w http.ResponseWriter, r *http.Request) {
	...
	if req.Username == "" || req.Password == "" {
		...
		json.NewEncoder(w).Encode(AttendanceResponse{Error: "Username and password are required"})
		return
	}
...
func handleTimetable(w http.ResponseWriter, r *http.Request) {
	...
	if req.Username == "" || req.Password == "" {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(TimetableResponse{Error: "Username and password are required"})
		return
	}
...
func handleHealth(w http.ResponseWriter, r *http.Request) {
	...
	w.Write([]byte(`{"status":"ok","message":"Server is running","timestamp":"` + time.Now().Format(time.RFC3339) + `"}`))
}
...
func Handler(w http.ResponseWriter, r *http.Request) {
	...
	switch {
	case path == "/attendance" || endpoint == "attendance":
		handleAttendance(w, r)
	case path == "/timetable" || endpoint == "timetable":
		handleTimetable(w, r)
	case path == "/health" || endpoint == "health":
		handleHealth(w, r)
	case path == "/" || path == "":
	...
	default:
		...
		response := map[string]interface{}{
			"message": "Vignan Student Portal API",
			"endpoints": map[string]string{
				"health":     "GET  /?endpoint=health",
				"attendance": "POST /?endpoint=attendance",
				"timetable":  "POST /?endpoint=timetable",
			},
			"status": "ok",
		}
